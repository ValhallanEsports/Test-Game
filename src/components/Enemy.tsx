...
import { getSafeSpawn } from './Player';
...
export function Enemy({ data }: { data: EnemyData }) {
  ...
  // Death pause and respawn when bot state is 'disabled'
  useEffect(() => {
    if (data.state === 'disabled') {
      // Calculate safe spawn far away from where the bot died
      const currentPos = data.position;
      const [rx, ry, rz] = getSafeSpawn(currentPos);
      
      // Instantly teleport the bot's rigid body to the new location
      if (body.current) {
        body.current.setTranslation({ x: rx, y: ry, z: rz }, true);
        body.current.setLinvel({ x: 0, y: 0, z: 0 }, true);
      }
      // Re-initialize patrol target near new spawn
      patrolTarget.current.set(
      ...
      const timer = setTimeout(() => {
        // Update store to reactivate the bot and set new position
        useGameStore.getState().respawnEnemy(data.id, [rx, ry, rz]);
  ...
  useFrame((state_fiber, delta) => {
    ...
    // Reset rotation and position on reactivation
    if (groupRef.current) {
