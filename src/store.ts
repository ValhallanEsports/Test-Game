...
interface GameStore {
  ...
  respawnEnemy: (id: string, position: [number, number, number]) => void;
...
export const useGameStore = create<GameStore>((set, get) => ({
    ...
    newSocket.on('gameJoined', (players: Record<string, PlayerData>) => {
      ...
      newSocket.on('playerHit', (data: { targetId: string, shooterId: string, targetDisabledUntil: number, shooterScore: number }) => {
        set(state => {
          ...
          const eventMsg = `${shooterName} eliminated ${targetName}`;
  ...
  hitPlayer: () => set((state) => {
    ...
    return {
      ...
      playerDisabledUntil: 0, // Instantly active or immediately recoverable
      lastHitTime: now,
      score: Math.max(0, state.score - 50), // Penalty for getting hit
      events: [...state.events, { id: Math.random().toString(), message: `You were eliminated!`, timestamp: now }]
    };
  }),
  ...
  hitEnemy: (id, byPlayer = false) => set((state) => {
    ...
    const enemies = state.enemies.map(e => {
      if (e.id === id && e.state === 'active') {
        return { ...e, state: 'disabled' as EntityState, disabledUntil: 0 };
      }
      return e;
    });
    return {
      ...
      events: byPlayer ? [...state.events, { id: Math.random().toString(), message: `You eliminated ${id}`, timestamp: Date.now() }] : state.events
    };
  }),

  respawnEnemy: (id, position) => set((state) => {
    const enemies = state.enemies.map(e => {
      if (e.id === id) {
        return { ...e, state: 'active' as EntityState, position, disabledUntil: 0 };
      }
      return e;
    });
    return { enemies };
